...
> **移至新設的 Phase 6(上線後擴充)**;入口頁改以「建設中」佔位卡預告(不進 manifest、
> 不可點,規格已入 §2.1/§7/§9,**尚未實作**)。測試指南 Phase 4 驗收清單已展開
> (13 條,含佔位卡 2 條),回應 verifier P2 發現。
> 下一步:實作佔位卡(Phase 4 收尾項)→ 安排 verifier 覆核 → Phase 5 上線。
...
| 4 | 題庫內容(站方決策縮減為單庫 + 佔位卡) | 🔶 daily-life 完成 + demo 下架;「建設中」佔位卡待實作 |
...
實作入口頁「建設中」佔位卡(Phase 4 收尾項,規格 §2.1、契約見方針 Phase 4 註記),
之後安排 verifier 覆核累積 phase(1 / 2 / 2.6 / 3 / 4,Phase 4 清單已入測試指南),
再進 Phase 5 上線。另兩庫(暫名「美食」「娛樂」)為 Phase 6,上線後逐題討論建題。
